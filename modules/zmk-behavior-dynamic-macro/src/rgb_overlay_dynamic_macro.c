/*
 * Copyright (c) 2026 The ZMK Contributors
 *
 * SPDX-License-Identifier: MIT
 *
 * RGB matrix overlay kind for the dynamic macro recorder: paints its targeted
 * LEDs while &dm is recording a take. Living in the dynamic-macro module proves
 * the public overlay API (KP_RGB_OVERLAY_DEFINE + kp_rgb_overlay_paint) is
 * enough for a third-party kind, with no edit to zmk-rgb-matrix.
 *
 * The recorder only exists on the split central (CONFIG_ZMK_BEHAVIOR_DYNAMIC_
 * MACRO is gated that way). The device still has to exist and link on the
 * peripheral, because the shield's shared kp-rgb.dtsi lists it on &kprgb for
 * both halves. The node is declared `central-authoritative`, so the central
 * evaluates `active` and pushes the result over the split link; the peripheral
 * gates its renderer on that pushed state instead of calling the getter.
 */

#define DT_DRV_COMPAT keypaw_rgb_overlay_dynamic_macro

#include <zephyr/device.h>

#include <zmk/rgb_matrix.h>

#include "dynamic_macro_state.h"

#if DT_HAS_COMPAT_STATUS_OKAY(DT_DRV_COMPAT)

struct kp_ovl_dm_config {
  struct kp_rgb_overlay_common_config common;
};
struct kp_ovl_dm_data {
  struct kp_rgb_overlay_common_data common;
};

static bool kp_ovl_dm_active(const struct device *dev) {
#if IS_ENABLED(CONFIG_ZMK_BEHAVIOR_DYNAMIC_MACRO)
  ARG_UNUSED(dev);
  return zmk_dynamic_macro_is_recording();
#else
  /* Peripheral: no recorder here. The engine never calls this there because the
   * node is central-authoritative (the central pushes the state), but the unit
   * still has to compile and link. */
  ARG_UNUSED(dev);
  return false;
#endif
}

static void kp_ovl_dm_render(const struct device *dev, struct kp_rgb_frame *frame) {
  /* The engine calls this only while kp_ovl_dm_active() is true. */
  const struct kp_ovl_dm_config *cfg = dev->config;
  const struct kp_ovl_dm_data *data = dev->data;

  kp_rgb_overlay_paint(frame, data->common.leds, data->common.led_count,
                         kp_hex_to_rgb(cfg->common.color), cfg->common.brightness);
}

#define KP_OVL_DM_DEFINE(inst)                                                 \
  KP_RGB_OVERLAY_TARGET_ARRAYS(inst, kp_ovl_dm_##inst);                      \
  static const struct kp_ovl_dm_config kp_ovl_dm_##inst##_cfg = {              \
      .common = KP_RGB_OVERLAY_COMMON(DT_DRV_INST(inst), kp_ovl_dm_##inst),  \
  };                                                                           \
  static struct kp_ovl_dm_data kp_ovl_dm_##inst##_data;                        \
  KP_RGB_OVERLAY_DEFINE(inst, kp_ovl_dm_active, kp_ovl_dm_render,            \
                          kp_ovl_dm_##inst)

DT_INST_FOREACH_STATUS_OKAY(KP_OVL_DM_DEFINE)

#endif /* DT_HAS_COMPAT_STATUS_OKAY(DT_DRV_COMPAT) */
